...
// 大纲面板子系统(Markdown / WYSIWYG / PDF 三路径)
const outlineApi = createOutline({
  getCurrentFilePath: () => currentFilePath,
  getEditor: () => editor,
  getWysiwyg: () => wysiwyg,
  getMode: () => mode,
  getPdfIframe: () => _currentPdfIframe,
  getPdfSrcUrl: () => _currentPdfSrcUrl,
  getOutlineLayout: () => outlineLayout,
  getOutlineLastSignature: () => _outlineLastSignature,
  setOutlineLastSignature: (s) => { _outlineLastSignature = s },
  shouldUpdateOutlinePanel,
  syncDetachedOutlineVisibility,
...
titlebarStatusApi = createTitlebarStatus({
  ...
  fmtStatus,
  scheduleOutlineUpdate: () => outlineApi.scheduleOutlineUpdate(),
})
  ...
  if (enable) {
      ...
      try {
        ...
        try {
          ...
          if (outline && shouldUpdateOutlinePanel(outlineLayout, outline)) {
            ...
            outlineApi.renderOutlinePanel()
            outlineApi.ensureOutlineObserverBound()
            outlineApi.bindOutlineScrollSync()
          }
        } catch {}
      ...
      try {
        ...
        if (outline && shouldUpdateOutlinePanel(outlineLayout, outline)) {
          ...
          setTimeout(() => { try { outlineApi.renderOutlinePanel(); outlineApi.bindOutlineScrollSync() } catch {} }, 0)
        }
      } catch {}
    ...
    // 绑定标签页切换：目录 / 大纲
      ...
      function activateLibTab(kind: 'files' | 'outline') {
        try {
          ...
          if (kind === 'outline') { try { outlineApi.renderOutlinePanel() } catch {} }
        } catch {}
      }
...
async function renderPreview(opts?: RenderPreviewOptions) {
  ...
  try {
    ...
    try {
      ...
      try { outlineApi.renderOutlinePanel() } catch {}
    } catch {}
...
async function toggleMode() {
  ...
  try {
    ...
    if (outline && shouldUpdateOutlinePanel(outlineLayout, outline)) {
      _outlineLastSignature = ''
      outlineApi.renderOutlinePanel()
      if (mode !== 'edit') outlineApi.bindOutlineScrollSync()
    }
  } catch {}
  // 触发模式变更事件（专注模式侧栏背景跟随）
...
async function persistLibraryVisible() {
  ...
  async function setOutlineLayout(mode: OutlineLayout, persist = true): Promise<void> {
    ...
    try { if (outlineLayout !== 'embedded') outlineApi.renderOutlinePanel() } catch {}
  }
...
// 全局：确保编辑器键盘钩子仅绑定一次（切换文档/重开窗也生效）
  ...
  editor.addEventListener('input', () => {
    ...
    if (!wasDirty) titlebarStatusApi?.refreshTitle()
      ...
      try { outlineApi.scheduleOutlineUpdateFromSource() } catch {}
    }
    // 便签模式：内容一变就自动保存（防抖）
