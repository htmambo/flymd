...
// 库私有化 v2 起,数据按"通道"分散：
//   通道A（本模块,库内共享）：<库根>/.flymd/config.json —— 最近文件、库树排序、
//     文件夹排序、粘贴默认目录、扩展启用状态等。随库目录走,可被 WebDAV 同步。
//   通道B（系统层按库命名空间,本模块提供 libraryScopedKey 工具）：Store /
//     localStorage key 追加 `:<libId>` —— 主要承载 tabSession(按窗口+库隔离)。
//   通道C（src/core/libraryPrivate.ts,库内私有,PR-3+）：<库根>/.flymd/local.json
//     —— docPos（光标位置）、uploader 凭据、插件 per-library 数据等设备私有
//     或敏感数据。**不**被 WebDAV 同步(PR-1 默认排除)。
//
// 通道A 生效条件=当前有库根（持久化库或临时库均可），无库才回落全局；
// 通道C 仅持久化库使用,临时库/无库一律回落全局(无持久化 = 走旧 Store 兜底)。
//
