    ...
    try {
      ...
      if (cfg) {
        if (Array.isArray(cfg.recent)) {
          return cfg.recent.filter((p) => typeof p === 'string')
        }
        // 播种：全局列表中属于本库的路径
        const seeded = (await getGlobalRecentFiles(store)).filter((p) => isInside(scope.root!, normSep(p)))
 ...
 * store 为 null 时静默忽略；持久化库激活时写入库内配置
 */
