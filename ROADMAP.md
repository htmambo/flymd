...
## 更新 v1.4.4
- 新增：主题设置加入「默认显示大纲」开关，开启后左侧库面板启动时默认打开大纲页签，而不是目录页签
- 新增：主题设置加入「关闭更新检测」开关，开启后启动静默检测和手动更新按钮都不再检查新版本
- 修复：PDF 导出与打印时统一列表缩进与 marker 间距，避免无序列表圆点侵占后方内容空间
- 修复：所见模式与阅读模式的加粗边界兼容性，中文标点、引号和括号包裹的加粗内容可以更稳定渲染

## 更新 v1.4.3
- 修复：临时库激活时，所有 WebDAV 同步入口都会在同步核心层直接跳过，包括启动同步、F5、设置页测试同步和菜单同步，避免同步仍作用到上一个正式库
- 修复：WebDAV 服务维护、网络中断或远端目录扫描失败时，本轮同步会直接跳过，不再把连接失败误判为空远端目录，从而避免错误触发本地删除或远端删除确认
- 修复：通过系统双击或“打开方式”启动并打开库外文档时，会先建立临时库会话再初始化启动同步，消除启动竞态导致的误同步风险
- 改进：WebDAV 同步增加会话级门禁回调，后续特殊库状态可在同步入口统一拦截，减少只在 UI 按钮上补丁式禁用造成的漏网入口

## 更新 v1.4.2
- 新增：阅读模式下可直接开启「源码 + 阅读分屏」，会自动切回源码并进入分屏，减少修改阅读内容时的模式切换步骤
- 修复：网络代理设置不再全局改写 `window.fetch`，并为插件 HTTP 与更新检查增加超时和本机地址绕过，避免启用代理后应用严重卡顿或启动后持续受影响
- 修复：拖入目录或异常目录时不再当作 Markdown 文档打开，并跳过 `EBWebView`、`node_modules`、`.git` 等高风险目录，避免侧栏扫描异常和卡顿
- 修复：暗色模式下库侧栏当前 Markdown 文档图标与选中态对比度更清晰
- 改进：库侧栏扫描对构建缓存、版本控制目录与 WebView 数据目录更保守，降低误扫临时/缓存目录的风险

## 更新 v1.4.1
- 新增：通过系统双击或“打开方式”打开库目录外的 `md` / `markdown` / `txt` 文档时，会话内自动把该文件所在目录作为临时库显示在侧栏；关闭应用或切回正式库后不写入库配置
- 改进：临时库侧栏会展示同目录下内置支持的 `md` / `markdown` / `txt` / `pdf` 文件以及插件扩展后缀，并在标题中标记“临时”，避免和正式库混淆
- 改进：临时库状态下禁用 WebDAV 同步入口，避免侧栏显示临时目录但同步仍作用到旧正式库
- 新增：主题设置加入符号自动补全开关，可按需关闭成对符号补全
- 修复：所见模式代码块粘贴后高亮层可正常刷新，避免粘贴代码不渲染或显示状态滞后
- 改进：库侧栏 Markdown / PDF 文件图标与选中态样式调整，当前文件识别更清晰
- 文档：移除 README 中失效或不再推荐的 PackyCode 中转服务链接

## 更新 v1.4.0
- 修复：关闭程序前会检查所有打开标签的未保存状态，不再只看当前文档；选择“保存并退出”时会逐个保存未保存标签，降低多标签编辑时丢改动风险
- 修复：所见模式中包含 `\$`、Excel 公式或其他 `$` 内容的段落不再被自动 Markdown 同步误改写，避免反斜杠丢失或污染加粗等 Markdown 标记
- 改进：源码模式 textarea 撤销/重做始终由每个标签独立的自定义撤销栈接管，并优化连续输入/删除的合并策略，避免回退到浏览器原生撤销栈造成历史错乱
- 调整：自动语音笔记暂时下线，仅隐藏插件菜单入口；底层功能实现保留，后续可按需恢复

## 更新 v1.3.9
- 新增：粘贴网页富文本或 Markdown 中的远程图片时，可自动下载到当前文档的 `images/` 目录并改写为相对路径，减少外链失效与离线不可用
- 改进：源码模式与所见模式共用图片落地逻辑，并在主题设置新增开关控制远程图片本地化行为

## 更新 v1.3.8
- 修复：文章预览中的目录锚点跳转现在可正确定位到对应标题，避免点击目录后无响应或跳转位置不准
- 改进：源码模式长文本性能继续优化，减少超长文档下滚动、选区变化、行号刷新和输入补丁路径的重复计算

## 更新 v1.3.7
- 改进：源码模式长文本性能继续优化，状态栏行列计算、标题刷新、源码大纲扫描等高频路径改为复用缓存，长文档下输入与界面更新更稳
- 改进：源码行号改为按可视区域增量测量与渲染，滚动、点击、选区变化不再反复重建整列 DOM，开启行号时对超长文档更友好
- 新增：主题设置加入「源码行号」开关，可按需关闭行号显示，进一步降低长文本编辑开销
- 改进：输入法补丁回退路径减少整篇 diff 扫描，中文等输入场景下的成对补全在长文本里响应更快

## 更新 v1.3.6
- 修复：预览中的本地文档链接现在可正确解析并打开相对路径目标，兼容 `md` / `markdown` / `txt` / `pdf`，并处理 `file://`、盘符路径与 UNC 路径等场景
- 修复：源码模式 Tab / Shift+Tab 缩进逻辑重新收敛，单光标、单行选区、多行选区行为统一，且兼容历史缩进 token 与原始制表符
- 修复：窗口最大化/还原按钮状态改为跟随真实窗口状态同步，普通窗口与专注模式下的标题栏按钮显示更可靠

## 更新 v1.3.5
- 改进：启动链路继续瘦身，主窗口改为首屏就绪后再显示；多标签、分屏、行号、库宽拖拽、主题面板等非关键模块延后加载，启动更快更稳
- 修复：源码模式中 `*` 与反引号补全改为即时处理，消除输入延时；单/双/三反引号包裹与代码围栏生成响应更直接
- 改进：Tab 缩进统一为四空格式缩进，并兼容旧版 `&emsp;` / 全角空格缩进；所见模式不再出现转译后的缩进残留
- 改进：源码模式行号样式进一步收敛，当前行提示更清晰
- 更新：扩展生态新增第三方插件「翻译选中文本」；「Blinko 集成」插件升级至 v1.2.0，支持发送文章、选择拉取、同步本地目录与生成登录门户文档

## 更新 v1.3.4
- 新增：源码模式加入左侧行号栏，随滚动同步，并高亮当前所在行
- 改进：Windows 安装包为关联文档使用独立文件图标，不再直接复用程序主图标
- 改进：Windows 文件关联统一收敛到 flyMD 自有 ProgID（如 `flymd.markdown` / `flymd.pdf`），避免污染系统通用类型名
- 其他：README 补充文件图标设计贡献者鸣谢

## 更新 v1.3.3
- 修复：便签模式在多显示器/混合缩放下高度自适应可能跳动，且内容变多时按钮区域遮挡正文
- 改进：便签模式测量高度前等待一帧，降低某些设备上高度抖动概率
- 修复：某些情况下新增库无反应（库路径选择兼容 `file://` / UNC 等返回值）
- 修复：所见模式代码块高亮层注释/引用斜体导致 Windows 下选区高亮错位（Refs #81）
- 修复：#80 F2 重命名复用弹窗，并支持 ESC 关闭
- 更新：内置插件「AI 小说引擎」v0.7.0：新增“严格遵循章节大纲”模式；世界设定与章节大纲支持文件夹；一键开坑完成页新增「生成第一章」按钮

## 更新 v1.3.2
- 修复：关闭应用前 WebDAV 同步可能不执行的问题
- 修复：缩放模式下窗口大小记忆异常
- 修复：Windows 缩放下便签窗口恢复位置/尺寸错位或变形
- 改进：便签模式预览隐藏时禁用高度调整，避免布局异常
- 改进：内置插件「AI 小说引擎」设置页增加“新用户免费获得50万字符”提示

## 更新 v1.3.1
- 新增：主题设置新增「网络代理」（HTTP/HTTPS），启用后插件网络请求也会走代理（桌面端），并可绕开部分 CORS 限制
- 新增：源码模式下，有序/无序列表回车自动续写表头；有序列表编号自动递增；空项回车退出列表
- 更新：内置插件「flymd-RAG 知识库索引」v0.1.6：支持 Office（Word/Excel：docx/xlsx/xls）转换为 Markdown 后建立向量索引并语义检索

## 更新 v1.3.0
- 新增：AI 助手快捷动作「摘要」：无选区时默认对当前文档生成摘要
- 改进：编辑器文末留白（Scroll past end）+ 接近底部输入时自动吸底，避免最后一行贴底
- 修复：右键菜单三级子菜单兼容（待办二级菜单不再因滚动而裁剪）
- 更新：内置插件「文档脑图（Markmap）」v0.2.1：节点文本自动换行；新增“AI提纲/原文”切换（仅用于脑图渲染，不改文档）；导出 SVG/PNG 也支持换行
- 更新：内置插件「xxtui 待办推送」v0.2.2：推送标题/内容模板（`$title`/`$content`）；支持普通文本推送；新增强制推送与“保留尾部标记”开关

## 更新 v1.2.9
- 新增：内置插件「文档脑图（Markmap）」v0.2.0：将当前 Markdown 文档渲染为脑图（右侧停靠预览/全屏遮挡/导出 SVG/PNG/点击节点跳转）
- 新增：库树右键菜单新增「在资源管理器中打开」（文件打开所在目录；文件夹直接打开）
- 改进：标签过多时自动压缩标签宽度，保证新打开的标签始终可见（Obsidian 风格）
- 改进：文档脑图面板支持拖拽调整宽度，并记住面板宽度

## 更新 v1.2.8
- 新增：库侧栏支持拖拽调整宽度

## 更新 v1.2.7
- 改进：库新增/删除/重命名统一移动到「库设置」窗口，删除仅从列表移除，不会删除磁盘文件
- 修复：双击放大/下拉还原等窗口操作异常
- 改进：新建文档/文件夹/重命名对话框禁用“点击遮罩关闭”，避免鼠标框选文字时误关窗口

## 更新 v1.2.6
- 修复：Windows 透明窗口在拖动/切换焦点/跨屏缩放后偶发“顶部白条/残影”（便签窗口也覆盖）
- 改进：便签模式实时自动保存（防抖 + 串行写入），失焦/切回阅读前强制落盘，降低丢改动概率
- 改进：便签模式隐藏右下角通知/宽度气泡，减少打扰；便签提示气泡支持自动换行避免溢出
- 更新：内置插件「PDF及图片高精度解析（pdf2doc）」v0.2.8（分割阈值提升到 500 页；分割提示与默认自动合并策略优化）

## 更新 v1.2.5
- 新增：PDF 导出进度遮罩（阶段/进度/耗时/实时日志），并支持一键终止
- 修复：长文档导出 PDF 偶发黑页/黑屏（分页渲染避免超长画布）
- 新增：所见模式表格右键菜单支持删除行/列
- 改进：多库切换器支持“垂直标题栏 / 侧栏内”两种显示位置，显示/隐藏逻辑收敛，库设置界面样式与文案优化
- 改进：库目录刷新去掉“空白帧”闪烁（保留旧树直到新树就绪）
- 改进：PDF.js 懒加载缓存与 worker 配置兜底，提升插件页数读取稳定性
- 修复：阅读模式 `Ctrl+A` 会误选中库名的问题
- 更新：内置插件「PDF及图片高精度解析（pdf2doc）」v0.2.7（本地化图片默认写相对路径；图片文件名推断更稳；PDF.js worker 兼容）
- 更新：内置插件「AI 小说引擎」v0.6.9（新增“全局硬约束”资料文件并自动注入）

## 更新 v1.2.4
- 新增：本地图片插入支持写入相对路径（例如 `images/xxx.png`），更适合跨设备同步（可在图床设置中开启）
- 新增：本地图片保存路径策略优化：优先保存到当前文档同目录 `images/`；未保存文档时使用默认粘贴目录（兜底用户图片目录）
- 新增：库设置对话框：支持库侧栏显示开关、库顺序自定义、当前库 WebDAV 开关与远端路径
- 改进：库侧栏库列表支持同时显示多个库并快捷切换，样式优化
- 改进：WebDAV 同步默认远端路径按当前库名/路径生成（对已配置用户保持 `/flymd` 兼容）
- 更新：内置插件「图床相册」v0.1.8（支持按时间排序）
- 更新：内置插件「AI 助手」v0.4.6（样式调整）
- 更新：内置插件「AI 小说引擎」v0.6.8
- 修复：Linux 菜单栏显示异常；Windows 启动时隐藏菜单栏但保留快捷键

## 更新 v1.2.3
- 新增：WebDAV 同步日志内置查看器（最新同步段落高亮；一键清空；支持设置日志保留天数并自动裁剪）
- 优化：内置插件「Git 版本管理」v0.1.4（新增 Ribbon 快捷按钮；Windows 下拉起 git 进程减少终端闪烁）
- 更新：内置插件「PDF及图片高精度解析（pdf2doc）」v0.2.6（菜单整理；批量解析后可选自动合并分割片段；页数获取失败可重试）
- 更新：内置插件「图床相册」v0.1.7（新增 Ribbon 快捷按钮）
- 更新：内置插件「笔记与任务日历」v0.1.1（新增 Ribbon 快捷按钮打开任务与日历面板）
- 更新：内置插件「Markdown 表格助手」v0.1.6（移除启动时“已激活”通知，减少打扰）
- 优化：插件菜单管理界面样式调整（更清晰）
...
- 优化：所见模式 KaTeX 渲染改为可视区域附近再渲染，并对小公式缓存，提升大文档性能
- 修复：阅读模式中将大纲剥离到右侧后，预览滚动条被遮挡导致不可见的问题

## 更新 v1.2.0
- 新增：PDF 预览支持 `Ctrl+Shift+P` 呼出命令面板（桌面端原生层捕获并转发）
- 改进：库侧栏条目悬浮显示完整文件名/路径（含后缀），避免省略号导致难以辨认
- 更新：PDF2Doc 插件 v0.1.8（多密钥配置与自动轮换；合计剩余页数与余额入口；解析前额度风险提示；大文本解析按人物分组；文案优化）
- 改进：插件 API 新增 `getPdfPageCount`（用于 PDF 页数预估与额度提示）

## 更新 v1.1.9
- 新增：WebDAV 加密盐值远端共享，新设备自动对齐盐值，避免跨设备解密失败
- 改进：WebDAV 同步冲突处理增加"应用到所有文件"选项
- 改进：跨设备同步后，本地图片路径自动映射到"当前文档目录 images/"，图片在新设备上仍然可见（不修改原文档内容）

## 更新 v1.1.8
- 新增：在线公告系统（后台拉取公告 JSON，右下角通知弹出，支持一键打开链接）
- 新增：阅读模式右键菜单「复制为富文本（内嵌图片）」，用于复制图片本体而非图片代码
- 优化：所见模式中间态进入首帧渲染时隐藏内容以减少闪烁
- 更新：editor-enhancer v0.1.3（Slash/包裹指令更新；新增日记/会议记录模板）
- 更新：AI 小说引擎 v0.6.6（索引数据备份/恢复逻辑改进）
- 更新：图床相册 v0.1.6（ImgLa/Lsky 兼容与分页/加载更多改进，支持自定义 baseUrl）
- 更新：Typecho 博文管理 v0.1.5（新增图床迁移）
